...
static float TIMER_LIMIT = 900;
Boolean wasKeyPressedLastFrame = false;
Display display;
...
void setup() {
  ...
  windowTitle("RGB gaming Calculator");
...
void shuffleButtons(){
  ...
  for(int i = 0; i<buttons.length; i++){
      if(buttonsList2.hasValue((int)buttons[i].getKey())){
        buttons[i].setChar((char)buttonsList2.get(current));
        current++;
        if(int(random(0, 40)) == 3){
          buttons[i].setChar(' ');
        }
      }
  }
...
void draw() {
  ...
  int percentage = (int)(((TIMER_LIMIT-(float)frameCount)/TIMER_LIMIT)*100);
  float hue = (frameCount * 5) % 361; 

        // 2. Set the background color using the calculated hue. 
        // Saturation 80, Brightness 90 for a bright, but not overly harsh, color.
        if(percentage == 67 || percentage == 42){
            background(0, 0, 255);
            return;
        }
  ...
  if(percentage <= 0 && percentage >= -3){
    text("0%", SCALE*23, SCALE*10);
    return;
  }
  textSize(SCALE*1.5);
  if(percentage > -4)
    text(percentage+"%", SCALE*23, SCALE*10);
  if((frameCount) >= TIMER_LIMIT){
    //PImage crasher;
    //crasher = loadImage("ajhfjkasf");
    //image(crasher, 0, 0);
  }
  ...
  for (int i = 0; i < buttons.length; i++) {
    ...
    if(clicked != 0)
       ;
    else if(keyPressed&&wasKeyPressedLastFrame==false){
      clicked = key;
